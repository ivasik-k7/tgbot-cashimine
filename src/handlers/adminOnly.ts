...
export function createAdminOnly(options?: AdminOnlyOptions): Middleware<BotContext> {
  const {
  ...
  return async (ctx, next) => {
    ...
    if (!userId) {
      ...
      await ctx.reply("❌ Unable to verify user identity.", { parse_mode: "HTML" });
      return;
    }
    ...
    const isSuperAdmin = superAdminIds.length > 0 
      ? superAdminIds.includes(userId)
      : isUserAdmin; // If no superAdminIds specified, all admins are super admins

    if (requireSuperAdmin && !isSuperAdmin) {
      logger.warn(`Super admin required, but user ${userId} is not a super admin`);
      await ctx.reply("🚫 This command requires super administrator privileges.", {
        parse_mode: "HTML"
      });
      return;
    }
    ...
    if (!isUserAdmin) {
      ...
      // Log the attempt
      logger.info(`Admin access denied for user ${userId} in chat ${ctx.chat?.id}`, {
        userId,
        chatId: ctx.chat?.id,
 ...
 */
export function checkIsSuperAdmin(ctx: BotContext, superAdminIds: number[] = []): boolean {
  const userId = ctx.from?.id;

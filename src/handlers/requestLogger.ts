...
export function createRequestLogger(options?: LoggingOptions): Middleware<BotContext> {
  const {
  ...
  return async (ctx, next) => {
    ...
    if (logFullUpdate && config.environment === 'development') {
      const sanitizedUpdate = sanitizeData(ctx.update, sensitiveFields, excludeSensitive);
...
function sanitizeData(data: any, sensitiveFields: string[], excludeSensitive: boolean): any {
